  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="space-y-6">
        <h2 className="text-base font-semibold text-white">Archivos Procesados ({sources.length})</h2>
              ...
              {files.map((f) => (
                <div key={f.filename} className="rounded-lg border border-slate-800 bg-slate-900/40 px-4 py-3 flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 font-mono truncate">{f.filename}</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${
                      f.filename.includes("_pdf")
                        ? "border-purple-800 text-purple-400 bg-purple-950/40"
                        : "border-emerald-800 text-emerald-400 bg-emerald-950/40"
                    }`}>
                      {f.filename.includes("_pdf") ? "PDF" : "Excel"}
                    </span>
                    {f.originalUrl && (
