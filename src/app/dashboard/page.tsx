...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/dashboard-data").then((r) => r.json()).then(setData).catch(console.error);
  }, []);

  if (!data) return <Loading />;

  const { trend, cantons, provinces, crimeTotals, stats } = data;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <ChartCard title="Tendencia Anual por Categoría de Delito" subtitle="2018-2022: tasas /10k (Excel) | 2023-2025: conteos reales (PDF Atlas)">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={trend} margin={{ left: -10, right: 10, top: 4, bottom: 0 }}>
            ...
            {selectedCrimes.map((ct) => (
              <Line key={ct} type="monotone" dataKey={ct} stroke={CRIME_COLORS[ct]} strokeWidth={2}
                dot={{ fill: CRIME_COLORS[ct], r: 3 }} name={ct} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={trend} margin={{ left: -10, right: 10, top: 4, bottom: 0 }}>
            ...
            <Bar dataKey="total" radius={[4, 4, 0, 0]}>
              {trend.map((pt, i) => (
                <Cell key={i} fill={pt.year >= 2023 ? "#ef4444" : "#64748b"} fillOpacity={0.85} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        <p className="text-xs text-slate-600 mt-2">Barras rojas = datos extraídos de PDF (2023+). Barras grises = Anexos Excel.</p>
      </ChartCard>

      {/* Crime breakdown + Province comparison side by side */}
      <div className="grid lg:grid-cols-2 gap-6">
        <ChartCard title="Distribución por Tipo de Delito" subtitle="Total acumulado 2018–2025">
        ...
        <ChartCard title="Comparativa Provincial" subtitle="Total acumulado por provincia">
          <ResponsiveContainer width="100%" height={240}>
      ...
      <ChartCard title={`Top 25 Cantones por Delitos Totales`} subtitle={`De ${stats.cantonCount} cantones con datos disponibles`}>
        <ResponsiveContainer width="100%" height={400}>
          <BarChart data={topCantons.map((c) => ({ name: c.canton.length > 14 ? c.canton.slice(0, 13) + "…" : c.canton, total: c.total, prov: c.province }))}
      ...
      {/* Full province × crime-type table */}
      <div>
...
function Loading() {
  return (
    <div className="flex items-center justify-center h-64">
      <div className="text-slate-400 text-sm animate-pulse">Cargando datos…</div>
    </div>
  );
}
